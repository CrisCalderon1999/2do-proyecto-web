...
<div class="row">
  ...
  <div class="card-body">
    <h5 class="font-semibold text-xl text-gray-800 leading-tight">Inventario de Presentacion</h5>
    <br>
    <a href="{{route('laboratorio.create')}}" class="bg-indigo-500 px-12 py-2 rounded text-gray-200 font-semibold hover:bg-indigo-800 transition duration-200 each-in-out">Agregar presentacion</a>
                  ...
                  <td >{{$item->Presentacion}}</td>
                      <td>
                        <form action="{{route('presentacion.edit',$item->id)}}" method="GET">
                          ...
                          <td>
                          <form action="{{ route('presentacion.delete', $item->id) }}" method="POST" class="formEliminar">
                                    @csrf
                                    @method('DELETE')
  ...
  Array.prototype.slice.call(forms)
    .forEach(function (form) {
      form.addEventListener('submit', function (event) {        
          ...
          Swal.fire({
                title: '¿Esta seguro de eliminar el registro "{{$item->Presentacion}}"?',        
                icon: 'info',
